...
type private Description private () =
    ...
    static member Of(diagnostic: Diagnostic): string =
        ...
        // Test0.cs(3,35): Error MustUseReturnValue : `Foo.Bar` must be invoked with named arguments
...
type DiagnosticAsserts private () =
    ...
    [<Extension>]
    static member AreEmpty(assertThat: IAssertThat<Diagnostic[]>): unit =
        Assert.That(assertThat.Actual).Match(Array.empty)


    [<Extension>]
    static member ContainNo(assertThat: IAssertThat<Diagnostic[]>,
                            diagnosticId: string): unit =
        for diagnostic in assertThat.Actual do
            if diagnostic.Id = diagnosticId
            then
                let message =
                    StringBuilder()
                        .Append($"Expected no '{diagnosticId}' diagnostics got:")
                        .AppendLine()
                        .Append(Description.Of(diagnostic))
                        .AppendLine()
                        .ToString()
